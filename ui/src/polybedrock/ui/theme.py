...
theme.py
────────
Centralised mutable font instances and colour palette for a Forge app.
...
#: Only the "classic" label carries the product name; the other four are
#: already generic. Formatted at read time so PolyShield keeps the exact string
#: it shipped and PolyScour does not inherit it.
_APP_NAME = ""


def configure(app_name: str) -> None:
    """Name the product whose theme this is. Affects preset labels only."""
    global _APP_NAME
    _APP_NAME = app_name
...
def preset_names() -> list[tuple[str, str]]:
    ...
    return [(k, _PRESET_DISPLAY_NAMES[k].format(app=_APP_NAME).strip())
            for k in _PRESET_PALETTES]
